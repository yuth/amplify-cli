...
const runRequestVTL = (template, graphqlInfo) => {
  // log.info('loading request vtl', path.relative(process.cwd(), fullPath));
  const context = buildVTLContext(graphqlInfo);
  // const content = fs.readFileSync(template, 'utf8');
  const result = handleVTLRender(template, context, vtlMacros, graphqlInfo);
  return result;
};

const runResponseVTL = (template, graphqlInfo, result) => {
  // log.info('loading response vtl', path.relative(process.cwd(), tempate));
  const context = buildVTLContext(graphqlInfo, result);
  // const content = fs.readFileSync(tempate, 'utf8');
  return handleVTLRender(template, context, vtlMacros, graphqlInfo);
};
...
const generateTypeResolver = (
  ...
  try {
    ...
    const request = runRequestVTL(requestTemplate, resolverArgs);
