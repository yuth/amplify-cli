...
export const sanityCheckProject = async (
  ...
  if (cloudBackendDirectoryExists && buildDirectoryExists) {
    ...
    sanityCheckDiffs(diffs, current, next, diffRules);
  }
};
...
const cantHaveMoreThan500ResourcesRule = (diffs: Diff[], currentBuild: DiffableProject, nextBuild: DiffableProject): void => {
